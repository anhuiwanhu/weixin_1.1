					...
					<form id="sendForm" action="/defaultroot/workflow/sendnew.controller?modibutton=<%=(String)pageContext.getAttribute("modibutton") %>&isDossier=<%=(String)pageContext.getAttribute("isDossier") %>" method="post">
							<table style="border-collapse:separate;border-spacing: 0 5px;">							
								<%
									...
									System.out.println("----govDocFormList-"+govDocFormList);
								...
								<%				}
											}else if("goldGridId".equals(_map.get("keyId"))){

												
											}else if("wordType".equals(_map.get("keyId"))){
												
											}else{
												if("commentList".equals(_map.get("keyId")) || "custemFieldList".equals(_map.get("keyId"))){
												}else{
								%>				
													<tr>
														<td><span><%=_map.get("name")%></span></td>
								...
								<c:if test="${not empty govDocXml}">
									...
									<x:if select="$bd/referenceAccessory">
										<tr>
											<td><span>参考文件</span></td>
											<td><p>
												<c:set var="realname"><x:out select="$bd/referenceAccessory//@saveName"/></c:set>
												<c:set var="filename"><x:out select="$bd/referenceAccessory/text()"/></c:set>
												<jsp:include page="../common/include_download.jsp" flush="true">
									...
									<x:if select="$bd/referenceAccessoryDesc">
										<tr>
											<td><span><x:out select="$bd/referenceAccessoryDesc/@name"/></span></td>
											<td><p><x:out select="$bd/referenceAccessoryDesc/text()"/></p></td>
										</tr>
									</x:if>
