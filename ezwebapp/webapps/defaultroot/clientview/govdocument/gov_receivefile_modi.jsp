                  ...
                  <div class="form-table">
					<form id="sendForm" action="/defaultroot/workflow/sendnew.controller?modibutton=<%=(String)pageContext.getAttribute("modibutton") %>&isDossier=<%=(String)pageContext.getAttribute("isDossier") %>" method="post">
							<table style="border-collapse:separate;border-spacing: 0 5px;">
								...
								<%				}
											}else if("doComment".equals(_map.get("keyId"))){
								%>	
											<!--<c:if test="${commentFieldName !='' }">
												<tr>
													<td><span>${commentFieldName }</span></td>
													<td><p><textarea name='comment_input' class="edit-txta edit-txta-l" maxlength="300" placeholder="请输入文字"></textarea></p></td>
												</tr>	
											</c:if>-->
								<%			}else{
								...
								<c:if test="${not empty govDocXml}">
									...
									<x:forEach select="$govDoc//baseData" var="bd" >
										...
										<!--<x:if select="$bd/commentList">
											<c:set var="commentField"><x:out select="$bd/commentList//text()"/></c:set>
											<c:if test="${commentField == '' }">
											<x:forEach select="$doc//commentList/comment" var="cm" >
												<tr>
													<td>
														<span><x:out select="$cm//step/text()"/></span>
													</td>
													<td><p>
														<c:set var="commentDate"><x:out select="$cm//date/text()"/></c:set>
														<x:out select="$cm//content/text()"/>&nbsp;&nbsp;<x:out select="$cm//person/text()"/><c:if test="${not empty commentDate}">(${commentDate})</c:if></br></p>
													</td>
												</tr>
											</x:forEach>
											</c:if>
